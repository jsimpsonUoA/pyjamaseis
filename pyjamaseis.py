...
def analyseTimes(data):
    diff = 0
    maxInc = 0
    for i in range(len(data)):
        if i != len(data)-1:    
            diff+=data[i+1]-data[i]
            maxInc = max(maxInc, data[i+1]-data[i])
    diff = diff/(len(data)-1)
    max_ = 0
    count = 0
    for i in range(len(data)):
        if i != len(data)-1: 
            if abs(data[i+1]-data[i]-diff) > max_:
                max_ = abs(data[i+1]-data[i]-diff)
                maxi = i
                count += 1
            elif abs(data[i+1]-data[i]-diff) > 0.005:
                count+=1
            
    print 'Length:', len(data)
    print 'Average Delta:', diff
    try:
        print 'Maximum Deviation:', max_, '(Lower:', data[maxi], ', Upper:', data[maxi+1],')'
    except:
        print 'No Max'
    print 'Frequency:', 1/diff
    print 'Number of Deviated:', count
    print 'Maximum Interval:', maxInc
